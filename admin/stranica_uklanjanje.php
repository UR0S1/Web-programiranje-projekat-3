    ...
    if (!isset($_SESSION['autor_uloga'])) {
        ...
        if ($_SESSION['autor_uloga']!="admin") {
            ...
            if (mysqli_num_rows($rezultat) <= 0) {
                ...
                header("Location: stranice.php?poruka=Cтраница+ниjе+пронаħена");
                exit();
            } else {
                $stranica_id = $_GET['id'];
                $sql = "DELETE FROM stranica WHERE `stranica_id`='$stranica_id'";
                if (mysqli_query($veza, $sql)) {
                    header("Location: stranice.php?poruka=Страница+jе+уклонена");
                    exit();
                } else {
                    header("Location: stranice.php?poruka=ниje+могуhе+избрисати+страницу");
                    exit();
                }
            }
        }
